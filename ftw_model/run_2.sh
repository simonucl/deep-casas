    ...
    for delta in ${deltas[@]}; do
        ...
        for feature_encoding in ${encoding_methods[@]}; do
            for batch in ${batch_size[@]}; do
                python train.py --model BiLSTM --features ../hh_dataset/hh_npy/fib_hh1${i1}_feature.npy --activities ../hh_dataset/hh_npy/fib_hh1${i1}_activity.npy --feature_encoding ${feature_encoding} --delta ${delta} --file_ext '_merged_batch_first' --batch ${batch} --device "cuda:1"
            done
